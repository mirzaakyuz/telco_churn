...
def user_input_fetures():
    contract = st.sidebar.selectbox('Which type of contract has the customer?',('Month-to-month','One year','Two year'))
...
unpickled_df = pd.read_pickle("to_streamlit.pkl")

df_total = pd.concat([unpickled_df, input_df])
...
load_clf = pickle.load(open('model_ada_load.pkl', 'rb'))
